...
{ 
	...
	OLED_WR_Byte( 0x10 | ((x & 0xF0) >> 4)  , OLED_CMD);  //写入列的地址  低半字节
	OLED_WR_Byte( 0x00 | (x & 0x0F)         , OLED_CMD);//写入列的地址 高半字节
}   	     	  
...
{  
    ...
    for(n=0;n<132;n++)OLED_WR_Byte(0,OLED_DATA); //写入 0 清屏
...
void OLED_Init(void)
{
	...
	#else 
  OLED_WR_Byte(0x04,OLED_CMD);	//偏移2位地址
  #endif
